...
enabled=$(openclaw config get plugins.entries.supervisor-phase1.enabled 2>/dev/null | awk 'NF{line=$0} END{print line}')
gate=$(openclaw config get plugins.entries.supervisor-phase1.config.gateEnabled 2>/dev/null | awk 'NF{line=$0} END{print line}')
